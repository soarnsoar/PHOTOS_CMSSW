...
if '__main__' == __name__:    
    ...
    import math
#####
#This script is to add virtual Z in DY process because offshell Z are not stored in event records in MG5.
...
#(3)If not Z, define Z*
#(4) Momentum of Z* = sum[Momentum of l+,l-]
#(5) remove l+l- in event records
#(6) add Z* / l+/l- in this order.
...
#    lhe = EventFile('unweighted_events.lhe')
    ...
    output = open(lhefile+'_add_virtual_Z.lhe', 'w')
...
#        event.nexternal=Nptl
        ...
        ####If N_leptons ==2, => check if it is a Z* event, which means no intermediate boson in event records###
        if Nl == 2:
            ###Check the lepton pair is the same flavour, has the same mothers (but not Z boson) => Z* event
            if lm.pid == -lp.pid and lm.mother1 == lp.mother1 and lm.mother2 == lp.mother2 and lp.mother1.pid != 23 and lm.mother2.pid != 23:
                offshell_event=True
       ##declare Z*##
        Z_virtual= Particle(None,event)
        #######if Z* event#######
        if offshell_event:
            ...
            event.remove(lm)
            ###define Z*#### 
            Z_virtual.event = event
            Z_virtual.event_id=len(event)##ordering index
            Z_virtual.pid= 23##Z
            Z_virtual.status= 2
            Z_virtual.mother1=lm.mother1 ##set its mother to mother of l+l-
            Z_virtual.mother2=lm.mother2
            Z_virtual.color1 = 0
            Z_virtual.color2 = 0
            Z_virtual.px = lm.px+lp.px
            Z_virtual.py = lm.py+lp.py
            Z_virtual.pz = lm.pz+lp.pz
            Z_virtual.E = lm.E+lp.E
            Z_virtual.mass = math.sqrt(math.pow(Z_virtual.E,2)-math.pow(Z_virtual.px,2)-math.pow(Z_virtual.py,2)-math.pow(Z_virtual.pz,2))
            Z_virtual.vtim = 0
            Z_virtual.comment=''

            Z_virtual.helicity = 0 ##? 

            event.append(Z_virtual)

            lm.event_id=len(event)
            event.append(lm)
            lp.event_id=len(event)
            event.append(lp)
            ####set l+l-'s mother to Z*####
            lp.mother1=Z_virtual
            lp.mother2=Z_virtual

            lm.mother1=Z_virtual
            lm.mother2=Z_virtual
     ###increase Nptl to Nptl+1 due to the new virtual Z
